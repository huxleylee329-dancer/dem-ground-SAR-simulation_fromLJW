function [ Image,controlpoint,Slantrange,Imageslantrange,Imageheight,NewX,NewY,numcount,ImagePosX,ImagePosY,ImagePosZ,Vs_X,Vs_Y,Vs_Z,Imageheightx,Imageheighty,Imageheightz] ...
    = generateSLC0401(C,satelipos,fd0posn,DEM,sigma,RandAngle,Wlength,Drange,DAzimath,ImageAzimath,ImageRange,vpos,NewX1,NewY1)
...
phase_re=cos(RandAngle).*cos(-C./Wlength*Slantrange)-sin(RandAngle).*sin(-C./Wlength*Slantrange);
phase_im=cos(RandAngle).*sin(-C./Wlength*Slantrange)+sin(RandAngle).*cos(-C./Wlength*Slantrange);
% phase_re=cos(-C./Wlength*Slantrange);
% phase_im=sin(-C./Wlength*Slantrange);
phase_re=awgn(phase_re,10);%这里相当于热噪声；y = awgn(x,snr)将白高斯噪声添加到向量信号x中，在此表示加入随机相位
phase_im=awgn(phase_im,10);
...
for i=1:row    
    for j=1:clomn
        ...
        Vs_X(NewX1(i,j),NewY1(i,j))=Vs_X(NewX1(i,j),NewY1(i,j))+vpos(i,j,1);
        Vs_Y(NewX1(i,j),NewY1(i,j))=Vs_Y(NewX1(i,j),NewY1(i,j))+vpos(i,j,2);
        Vs_Z(NewX1(i,j),NewY1(i,j))=Vs_Z(NewX1(i,j),NewY1(i,j))+vpos(i,j,3);
