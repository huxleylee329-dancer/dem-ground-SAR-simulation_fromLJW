function [Gradiennt_jump,dz] = pdv_jump_mark_phase(PDV,image_phase)
%利用相位导数方差和跳变点定义标记不连续点

%利用相位导数方差和图像膨胀标记不连续点
pdv_threshold = 0.85;%相位导数方差阈值
[pr,pc] = size(PDV);
PDV(PDV>pdv_threshold)=1;%将大于阈值的点标为1
dz_pdv = zeros(pr,pc);
dz_pdv(PDV==1) = 1;%dy为全1矩阵
se = strel('disk',3);%垂直线结构元素:圆形
% se = strel('diamond',6);%垂直线结构元素：菱形
...
dz_jump(dz_jump>1)=1;
% se = strel('disk',1);%垂直线结构元素:圆形
% dz_jump = imdilate(dz_jump,se);

%取dz_pdv和dz_jump的并集
dz = dz_jump + dz_pdv(4:end-1,4:end-1);
dz(dz>1)=1;
