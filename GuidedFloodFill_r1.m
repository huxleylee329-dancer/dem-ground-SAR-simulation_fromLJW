...
function IM_unwrapped = GuidedFloodFill2(IM_phase, IM_mag, IM_unwrapped, unwrapped_binary, derivative_variance, adjoin, IM_mask,residuemat,C)

[r_dim, c_dim] = size(IM_phase);
indicator = 101;%101
% Include edge pixels
while sum(sum(adjoin(:))) ~= 0  %Loop until there are no more adjoining pixels
  adjoining_derivative_variance = derivative_variance.*adjoin + indicator.*~adjoin; %Derivative variance values of the adjoining pixels (pad the zero adjoining values with 100)
  min_deriv_var = min(adjoining_derivative_variance(:)); % the minimum derivative variance
  if(min_deriv_var >= indicator)  % 101 is an indicator of already assigned variance
    break;  % finished
  end
  ...
  if residuemat(r_active,c_active) ~=0
      IM_unwrapped(r_active, c_active) = IM_phase(r_active, c_active);
      adjoin(r_active,c_active)= 0;
      unwrapped_binary(r_active, c_active) = 1;
      IM_mask(r_active, c_active)  = 0 ;
      continue;
  end
  ...
  if(r_active+1<=r_dim)  % Is this a valid index?
    if unwrapped_binary(r_active+1, c_active)==1 && residuemat(r_active+1, c_active) ==0
...
%     else % unwrapped_binary(r_active+1, c_active)==0
    elseif(unwrapped_binary(r_active+1, c_active) == 0)
...
%     end
  ...
  if(c_active+1<=c_dim)  % Is this a valid index?
    if unwrapped_binary(r_active, c_active+1)==1 && residuemat(r_active, c_active+1) ==0
...
%     else % unwrapped_binary(r_active, c_active+1)==0
    elseif(unwrapped_binary(r_active, c_active+1) == 0)
