...
function [phase_filter] = Improved_Goldstein_filter(Original_Interferogram, Win_size)
%% %%空间自适应滤波
%窗参数
% Win_size=17;  
...
for ii = Radius+1:Ni-Radius
  for jj = Radius+1:Nj-Radius
      ...
      Conherence_Window = coherence_update(ii-1:ii+1,jj-1:jj+1);
      PSD_Window = PSD(ii-1:ii+1,jj-1:jj+1);
      Num=sum(sum((PSD_Window<0.2)));
      Conherence_Window=Conherence_Window.*(PSD_Window<0.2);
      Gamma=sum(sum(Conherence_Window))./Num;
      %频率估计前预滤波
      window_mean = Mean_Filter_Correlation(phase_estimation, Gamma);
      %FFT频率估计
%       phase_2fft = fftshift(fft2(window_mean,nn,nn)); %窗口内2DFFT
     ...
     %% 残留相位滤波
      ...
      phase_0 = Afilter_Goldstein_Correlation(phase_0_matrix, Gamma-sqrt((fii.^2+fii.^2)/2)); 
      % 初相 + 线性相位 
