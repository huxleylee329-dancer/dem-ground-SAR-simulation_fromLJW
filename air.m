...
disp('生成SLC图像');
%输出控制点及图像像素对应的轨道信息
[Image,conp1,Imageheight,IS1,~,NewX,NewY,~,MainPosX,MainPosY,MainPosZ,~,~,~]=generateSLC(satelipos,fd0pos,DEM,sigma,RandAngle,Wlength,Drange,DAzimath,ImageAzimath,ImageRange);%输出主图像及控制点信息
MainSLC=Image(startpix(1):endpix(1)-1,startpix(2):endpix(2)-1);
Imageheight_m=Imageheight(startpix(1):endpix(1)-1,startpix(2):endpix(2)-1);
...
%% 
[Image2,conp2,Imageheight,IS2,~,~,~,~,SlavePosX,SlavePosY,SlavePosZ,~,~,~]=generateSLC(satelipos,fd0pos1,DEM,sigma,RandAngle,Wlength,Drange,DAzimath,ImageAzimath,ImageRange,NewX,NewY);%输出无形变辅图像图像及控制点信息
SlaveSLC_ideal=Image2(startpix(1):endpix(1)-1,startpix(2):endpix(2)-1);%理想情况下的辅图像
Imageheight_s=Imageheight(startpix(1):endpix(1)-1,startpix(2):endpix(2)-1);
...
figure;imagesc(angle(exp(1i*flat_phase_master)));colorbar;
%% 辅图像去参考地形
display('辅图去平地......');
rows = GCPs_slave(:,4); 
cols =  GCPs_slave(:,5); 
...
%% 求相位误差
phase_real = (Slantrange_ideal/Wlength*4*pi - Slantrange_Main/Wlength*4*pi);
