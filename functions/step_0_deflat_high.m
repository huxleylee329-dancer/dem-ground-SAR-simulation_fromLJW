%---------------------------去除参考平面的相位（高频）--------------------------
para_path = '.\Kernel\parameter\high\';% 控制点参数路径
out_path = '.\Kernel\Deflat_output\high\';% 去平地结果保存路径 
SLC_path = '.\Kernel\Input\high\';% 主辅图像路径
warning off;
% 检查并读入参数
display('检查并读入参数......');
...
% 去平地效应
deflat_mast_high = mast.*exp(1i*flat_phase_master);
% 保存结果
display('保存结果......');
flat_phase_mast_high = flat_phase_master;
save([out_path 'flat_phase_mast_high.mat'], 'flat_phase_mast_high');% 保存绝对平地相位（后续加回来）
save([out_path 'deflat_mast_high.mat'], 'deflat_mast_high');% 保存去平地之后的主图像
fig1 = figure;
set(fig1, 'visible', 'off');
imagesc(angle(exp(1i*flat_phase_master)));colorbar;
saveas(fig1, [out_path 'flat_phase_high.jpg']);% 保存平地相位图片（缠绕后的）
display('完成！');
...
flat_phase_slave_high = flat_range_slave * 4 * pi / lambda2;  %主图平地相位
%去平地效应
deflat_slave_high = slave.*exp(1i*flat_phase_slave_high);
display('保存结果......');
save([out_path 'deflat_slave_high.mat'], 'deflat_slave_high');% 保存去平地之后的辅图像
save([out_path 'flat_phase_slave_high.mat'], 'flat_phase_slave_high');% 保存绝对平地相位
display('完成！');
display('去平地完成！');
