function psd = RES_PSD(phase) %[weight,weight_1] [WW0,WW1] 
% 残余相位计算psd

j=sqrt(-1);

winsize_e_i =11;           %估计窗i方向尺寸
winsize_e_j =11;           %估计窗j方向尺寸
winsize_f_i = 9;  %滤波窗i方向尺寸
winsize_f_j =9;  %滤波窗j方向尺寸0104修改
...
for ii = wine_i+1:Ni-wine_i
  for jj = wine_j+1:Nj-wine_j
      ...
      % 初相,sum(sum()) or mean(mean())
       ...
       phase_0_matrix = angle(phase_0_matrix);
       
       uniform_phase=mean(mean(phase_0_matrix(winf_i:winf_i+2,winf_j:winf_j+2))); 
       phase_c=phase_0_matrix(winf_i+1,winf_j+1);
       diff(ii,jj) = (phase_c-uniform_phase).^2;
...
close(wb);
%% 取复相角(rad),取有效范围
diff = diff(wine_i+1:Ni-wine_i,wine_j+1:Nj-wine_j);
psd=sqrt(sum(sum(diff)./(Ni*Nj-1)));
end